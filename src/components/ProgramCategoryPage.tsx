...
import { images, type Program } from "@/lib/content";
        ...
        <div className="luxury-container relative grid gap-8 pb-14 lg:grid-cols-[1fr_0.9fr] md:pb-18">
          ...
          <div className="grid gap-4 self-end sm:grid-cols-2">
            <div className="relative min-h-[500px] overflow-hidden rounded-[1.8rem] border border-white/60 bg-charcoal shadow-[0_18px_60px_rgba(45,45,45,0.08)] sm:col-span-2">
              <Image
                src={categoryLabel.includes("Therapy") ? images.founder : images.editorial}
                alt={categoryLabel.includes("Therapy") ? "Dr. Devanu in a calming portrait" : "Dr. Devanu in a reflective portrait"}
                fill
                sizes="(min-width: 768px) 40vw, 100vw"
                className={`object-cover ${categoryLabel.includes("Therapy") ? "object-[center_30%]" : "object-[center_18%]"}`}
              />
              <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(20,20,20,0.08)_0%,rgba(20,20,20,0.56)_100%)]" />
              <div className="absolute inset-x-0 bottom-0 p-6 text-white">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-white/60">
                  Guided Mentorship Pathways
                </p>
              </div>
            </div>
      ...
      <section className="pb-20 md:pb-24">
        <div
          className={`luxury-container grid gap-6 md:grid-cols-2 xl:grid-cols-3 ${
            programs.length % 3 === 2 ? "[&>*:last-child]:xl:col-span-2" : ""
          }`}
        >
